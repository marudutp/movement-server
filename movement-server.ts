...
interface PlayerData {
  ...
  audioSocketId?: string; // Untuk koneksi ke audio server
}
...
io.on('connection', (socket: any) => {
  ...
  socket.on('drawData', (data: any) => {
    console.log("📥 [SERVER] drawData received:", JSON.stringify(data)); // ← TAMBAHKAN
    const uid = socketUidMap.get(socket.id);
    const player = uid ? activeUsers.get(uid) : null;
...
// SERVER START
// ============================================
// const MOVEMENT_PORT = process.env.MOVEMENT_PORT || 8080;
// server.listen(MOVEMENT_PORT, '0.0.0.0', () => {
//     console.log("--------------------------------------------------");
